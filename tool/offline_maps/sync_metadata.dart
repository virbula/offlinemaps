...
Future<void> main(List<String> arguments) async {
  try {
    ...
    stderr.writeln('Metadata sync failed: ${error.message}');
    exitCode = 2;
  }
}
...
Future<void> syncMetadata({
  ...
  final tag = string(release['releaseTag'], 'releaseTag');
  ...
  final files = <String, File>{
    ...
    'build/expected/manifest-${release['releaseTag']}.json': File(
  ...
  try {
    ...
    final next = await api.post('/git/commits', <String, Object?>{
      'message': 'Sync offline map metadata $tag',
